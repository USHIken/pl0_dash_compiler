...
int compile()
{
    ...
    printf("[NOTE]end of blockBegin\n");

    block(0);
    printf("[NOTE]end of block\n");
    finalSource();
    i = errorN();
    if (i!=0)
        printf("%d errors\n", i);
...
void statement()
{
    ...
    while (1)
    {
        printf("[NOTE]at while() in statement()\n");
        switch (token.kind)
        {
        case Id:
            printf("[NOTE]start searchT()\n");
            tIndex = searchT(token.u.id, varId);
            printf("[NOTE]end of searchT()\n");
        ...
        case WriteLn:  
            ...
            genCodeO(wrl);
            return;
        case End: case Semicolon: case Period:
            return;
        default:
            errorDelete();
            token = nextToken();
